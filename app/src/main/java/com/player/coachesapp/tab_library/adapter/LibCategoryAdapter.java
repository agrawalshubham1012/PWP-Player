...
import com.player.coachesapp.tab_library.SubCategoryActivity;
...
public class LibCategoryAdapter extends RecyclerView.Adapter<LibCategoryAdapter.ViewHolder> {
    private Context context;
    private int lastPosition = -1;
    private List<CatSubCatModel> list;

    public LibCategoryAdapter(Context context, List<CatSubCatModel> list) {
        ...
        holder.viewLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                context.startActivity(new Intent(context, SubCategoryActivity.class)
                        .putExtra("KEY", list.get(position).getCatTitle())
                        .putExtra("ID", "" + list.get(position).getCategoryId()));
            }
        });
